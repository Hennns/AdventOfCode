...
case class Slope(stepsRight: Int, stepsDown: Int = 1) {
  ...
  private val puzzleArray: Array[Array[Char]] = puzzleInput.toCharArray.grouped(maxInputLengthBeforeRepeating).toArray


  def countTrees(): Long = {
    @tailrec
    def skipLines(i: Int, lst: List[Array[Char]], acc: List[Array[Char]]): List[Array[Char]] = (i, lst) match {
      case (_, Nil) => acc
      case (1, _ :: xs) => skipLines(stepsDown, xs, acc)
      case (i, x :: xs) => skipLines(i - 1, xs, x :: acc)
    }

    val filteredLines = if (stepsDown > 1 ) skipLines(stepsDown, puzzleArray.toList, Nil).reverse else puzzleArray.toList
...
object Slope{
  ...
  val answerPart2: BigInt = slope1.countTrees() * slope2.countTrees() * slope3.countTrees() * slope4.countTrees() * slope5.countTrees()
}
